...
void GLFontBase::SetScale(float newScale)
{
	scale = newScale;
}
...
void PixelPerfectGLFont::TextOut (const char* String, int x, int y, int z)
{
	...
	for (int i = 0; i < Length; i++)
	{
		...
		glVertex3i(x, y - Char->height * scale, z);
		glTexCoord2f(Char->tx2, Char->ty2);
		glVertex3i(x + Char->width * scale, y - Char->height * scale, z);
		glTexCoord2f(Char->tx2, Char->ty1);
		glVertex3i(x + Char->width * scale, y, z);
